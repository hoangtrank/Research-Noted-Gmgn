...
(() => {
  ...
  const en = {
    ...
    grok_auto_setting: 'Auto-save Grok answers into the timeline (tabs opened from a note)', grok_auto: 'Auto-save answers', grok_auto_wait: 'Auto-save on — send the prompt to Grok.', grok_auto_read: 'Auto-save on — reading the answer…', grok_auto_off: 'Auto-save off — use Capture, then Save.', grok_autosaved: 'Auto-saved to {symbol}', undo: 'Undo', grok_undone: 'Removed', grok_dup: 'Already saved',
  ...
  const vi = {
    ...
    grok_auto_setting: 'Tự lưu câu trả lời của Grok vào timeline (tab mở từ ghi chú)', grok_auto: 'Tự lưu câu trả lời', grok_auto_wait: 'Tự lưu đang bật — hãy gửi prompt cho Grok.', grok_auto_read: 'Tự lưu đang bật — đang đọc câu trả lời…', grok_auto_off: 'Tự lưu đang tắt — bấm Bắt câu trả lời rồi Lưu.', grok_autosaved: 'Đã tự lưu vào {symbol}', undo: 'Hoàn tác', grok_undone: 'Đã gỡ', grok_dup: 'Đã lưu trước đó',
  ...
  const zh = {
    ...
    grok_auto_setting: '自动把 Grok 的回答保存到时间线（从笔记打开的标签页）', grok_auto: '自动保存回答', grok_auto_wait: '自动保存已开启 —— 请先把提示词发给 Grok。', grok_auto_read: '自动保存已开启 —— 正在读取回答…', grok_auto_off: '自动保存已关闭 —— 请点击「捕获回答」再保存。', grok_autosaved: '已自动保存到 {symbol}', undo: '撤销', grok_undone: '已移除', grok_dup: '之前已保存',
