...
(() => {
  ...
  const en = {
    ...
    pin_title: 'Pin this project: it stays at the top of the list and its note button turns orange', copy_addr: 'Copy address', mc_now: 'Current MC ≈ {mc}', mc_short: 'MC',
    ...
    panel_empty_2: 'This panel sits outside the page, so it never covers gmgn. Drag its edge to resize.', sync_title: 'Sync between computers (optional)', sync_desc: 'Keeps your notes the same on every computer where you use Chrome with the same Google account. The notes go into a hidden file in YOUR OWN Google Drive that only this extension can read. Off by default; nothing is sent to Google unless you turn it on.', sync_on_btn: 'Sign in with Google and turn on sync', sync_now: 'Sync now', sync_off_btn: 'Turn off', sync_wipe: 'Delete the copy on Drive', sync_wipe_confirm: 'Delete the sync file from your Google Drive? The notes on this computer stay. The next computer that syncs will create the file again from its own notes.', sync_wiped: 'The copy on Google Drive was deleted.', sync_state_off: 'Sync is off.', sync_state_working: 'Syncing…', sync_state_ok: 'Last sync {when} · {n} projects', sync_state_never: 'Sync is on — not synced yet.', sync_denied: 'Permission was not granted, so sync stays off.', sync_err_signed_out: 'Not signed in to Google. Turn sync off and on again to sign in.', sync_err_quota: 'Your Google Drive is full.', sync_err_forbidden: 'Google Drive refused access (a work or school account may block it).', sync_err_network: 'Could not reach Google Drive. It will try again.', sync_err_bad_remote: 'The sync file on Google Drive is damaged. Your notes on this computer were not touched. Delete the copy on Drive to start over.', sync_err_newer: 'Another computer runs a newer version of the extension. Update this one.', sync_err_other: 'Sync failed: {msg}', sync_err_unpacked: 'Google sign-in only works in the build installed from the Chrome Web Store. This copy was loaded from a folder, so it has a different extension ID ({id}) and Google refuses it. Install from the Store, or see docs/DEVELOPMENT.md for running the source with the Store key.', backups: 'Automatic backups (taken before an import or a sync changes your notes)', backup_none: 'No backup yet', backup_restore: 'Restore', backup_item: '{when} · {n} projects · before {reason}', backup_r_import: 'import', backup_r_sync: 'sync', backup_r_restore: 'a restore', backup_confirm: 'Replace ALL current notes with this backup? The current notes are backed up first, so you can undo this.', backup_done: 'Restored {n} projects.', ext_reloaded: 'Research-Noted-Gmgn was updated. Reload this page to keep using it.', gmgn_new: 'New to gmgn.ai? Open it here',
    ...
    pinned_only: '📌 Pinned only', back_list: '← All projects', pinned_title: 'Pinned', add_url: '+ Add from gmgn URL', export_json_short: 'Export JSON', export_md: 'Export Markdown', import_json: 'Import JSON',
  ...
  const vi = {
    ...
    pin_title: 'Ghim dự án: luôn nằm đầu danh sách, nút ghi chú chuyển màu cam', copy_addr: 'Copy địa chỉ', mc_now: 'MC hiện tại ≈ {mc}', mc_short: 'MC',
    status: 'Trạng thái', conviction: 'Mức tin tưởng', conviction_title: 'Mức tin tưởng 1–5 (bấm lại sao đang chọn để bỏ)',
    ...
    panel_empty_2: 'Panel này nằm ngoài trang nên không che gmgn. Kéo mép để đổi độ rộng.', sync_title: 'Đồng bộ giữa các máy (tuỳ chọn)', sync_desc: 'Giữ ghi chú giống nhau trên mọi máy bạn dùng Chrome với cùng một tài khoản Google. Ghi chú nằm trong một file ẩn trên Google Drive CỦA CHÍNH BẠN, chỉ extension này đọc được. Mặc định tắt; không có gì được gửi tới Google nếu bạn không bật.', sync_on_btn: 'Đăng nhập Google và bật đồng bộ', sync_now: 'Sync ngay', sync_off_btn: 'Tắt', sync_wipe: 'Xoá bản trên Drive', sync_wipe_confirm: 'Xoá file đồng bộ khỏi Google Drive của bạn? Ghi chú trên máy này vẫn còn. Máy nào sync tiếp theo sẽ tạo lại file từ ghi chú của nó.', sync_wiped: 'Đã xoá bản trên Google Drive.', sync_state_off: 'Đồng bộ đang tắt.', sync_state_working: 'Đang đồng bộ…', sync_state_ok: 'Sync lần cuối {when} · {n} dự án', sync_state_never: 'Đồng bộ đang bật — chưa sync lần nào.', sync_denied: 'Bạn chưa cấp quyền, nên đồng bộ vẫn tắt.', sync_err_signed_out: 'Chưa đăng nhập Google. Tắt rồi bật lại đồng bộ để đăng nhập.', sync_err_quota: 'Google Drive của bạn đã đầy.', sync_err_forbidden: 'Google Drive từ chối truy cập (tài khoản công ty/trường có thể bị chặn).', sync_err_network: 'Không kết nối được Google Drive. Sẽ thử lại sau.', sync_err_bad_remote: 'File đồng bộ trên Google Drive bị hỏng. Ghi chú trên máy này không bị đụng tới. Bấm Xoá bản trên Drive để làm lại từ đầu.', sync_err_newer: 'Máy khác đang chạy bản extension mới hơn. Hãy cập nhật máy này.', sync_err_other: 'Đồng bộ thất bại: {msg}', sync_err_unpacked: 'Đăng nhập Google chỉ chạy với bản cài từ Chrome Web Store. Bản này nạp từ thư mục nên có mã định danh khác ({id}) và Google từ chối. Hãy cài từ Store, hoặc xem docs/DEVELOPMENT.md để chạy mã nguồn kèm key của bản Store.', backups: 'Bản sao lưu tự động (chụp trước khi import hoặc sync làm thay đổi ghi chú)', backup_none: 'Chưa có bản sao lưu nào', backup_restore: 'Khôi phục', backup_item: '{when} · {n} dự án · trước khi {reason}', backup_r_import: 'import', backup_r_sync: 'sync', backup_r_restore: 'khôi phục', backup_confirm: 'Thay TOÀN BỘ ghi chú hiện tại bằng bản sao lưu này? Ghi chú hiện tại được sao lưu trước, nên bạn vẫn quay lại được.', backup_done: 'Đã khôi phục {n} dự án.', ext_reloaded: 'Research-Noted-Gmgn vừa được cập nhật. Tải lại trang này để dùng tiếp.', gmgn_new: 'Chưa biết gmgn.ai là gì? Mở tại đây',
    ...
    language: 'Ngôn ngữ', reload_hint: 'Hãy tải lại trang gmgn rồi thử lại', stats: '{n} dự án · {p} đã ghim · {e} mốc',
    ...
    pinned_only: '📌 Chỉ ghim', back_list: '← Danh sách dự án', pinned_title: 'Đã ghim', add_url: '+ Thêm từ URL gmgn', export_json_short: 'Xuất JSON', export_md: 'Xuất Markdown', import_json: 'Nhập JSON',
    ...
    md_title: 'Research-Noted-Gmgn — export {date}', md_address: 'Địa chỉ', md_status: 'Trạng thái', md_conviction: 'Mức tin tưởng', md_tags: 'Tags', md_summary: 'Tóm tắt', md_timeline: 'Timeline',
  };
  ...
  const zh = {
    ...
    panel_empty_2: '此面板位于页面之外，不会遮挡 gmgn。拖动边缘可调整宽度。', sync_title: '多台电脑同步（可选）', sync_desc: '让你在所有使用同一 Google 账号登录 Chrome 的电脑上保持笔记一致。笔记保存在你自己的 Google Drive 中一个隐藏文件里，只有本扩展可以读取。默认关闭；不开启就不会向 Google 发送任何内容。', sync_on_btn: '登录 Google 并开启同步', sync_now: '立即同步', sync_off_btn: '关闭', sync_wipe: '删除 Drive 上的副本', sync_wipe_confirm: '从你的 Google Drive 删除同步文件？本机笔记会保留。下一台同步的电脑会用它自己的笔记重新创建文件。', sync_wiped: '已删除 Google Drive 上的副本。', sync_state_off: '同步已关闭。', sync_state_working: '正在同步…', sync_state_ok: '上次同步 {when} · {n} 个项目', sync_state_never: '同步已开启 —— 尚未同步。', sync_denied: '未授予权限，同步保持关闭。', sync_err_signed_out: '未登录 Google。请关闭再开启同步以登录。', sync_err_quota: '你的 Google Drive 已满。', sync_err_forbidden: 'Google Drive 拒绝访问（公司或学校账号可能被限制）。', sync_err_network: '无法连接 Google Drive，稍后会重试。', sync_err_bad_remote: 'Google Drive 上的同步文件已损坏。本机笔记未被改动。请删除 Drive 上的副本后重新开始。', sync_err_newer: '另一台电脑运行着更新版本的扩展，请更新本机。', sync_err_other: '同步失败：{msg}', sync_err_unpacked: 'Google 登录仅在从 Chrome 网上应用店安装的版本中可用。此副本从文件夹加载，扩展 ID 不同（{id}），Google 会拒绝。请从应用店安装，或参见 docs/DEVELOPMENT.md 使用应用店的 key 运行源码。', backups: '自动备份（在导入或同步改动笔记之前生成）', backup_none: '暂无备份', backup_restore: '恢复', backup_item: '{when} · {n} 个项目 · {reason}之前', backup_r_import: '导入', backup_r_sync: '同步', backup_r_restore: '恢复', backup_confirm: '用此备份替换当前全部笔记？当前笔记会先被备份，因此可以撤销。', backup_done: '已恢复 {n} 个项目。', ext_reloaded: 'Research-Noted-Gmgn 已更新，请刷新此页面后继续使用。', gmgn_new: '还不了解 gmgn.ai？从这里打开',
    ...
    pinned_only: '📌 仅置顶', back_list: '← 返回列表', pinned_title: '已置顶', add_url: '+ 从 gmgn 链接添加', export_json_short: '导出 JSON', export_md: '导出 Markdown', import_json: '导入 JSON',
