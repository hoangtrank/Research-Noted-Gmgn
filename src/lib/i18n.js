...
(() => {
  ...
  const en = {
    ...
    follow_page: 'Side panel follows the token you open (gmgn, DexScreener, X search)', list_badges_setting: 'Also show a ✎ button on every row of token lists (gmgn, DexScreener)',
    sel_save: 'Save selection → {symbol}', sel_saved: 'Saved to {symbol}', export_images: 'Include screenshots in JSON export', open_image: 'Open image', image_alt: 'Screenshot',
  ...
  const vi = {
    ...
    follow_page: 'Side panel tự chuyển theo token đang mở (gmgn, DexScreener, tìm kiếm X)', list_badges_setting: 'Hiện thêm nút ✎ ở từng hàng trong danh sách token (gmgn, DexScreener)',
    sel_save: 'Lưu đoạn bôi đen → {symbol}', sel_saved: 'Đã lưu vào {symbol}', export_images: 'Kèm ảnh chụp khi xuất JSON', open_image: 'Mở ảnh', image_alt: 'Ảnh chụp',
  ...
  const zh = {
    ...
    follow_page: '侧边栏跟随当前打开的代币（gmgn、DexScreener、X 搜索）', list_badges_setting: '在代币列表的每一行也显示 ✎ 按钮（gmgn、DexScreener）',
    sel_save: '保存选中文本 → {symbol}', sel_saved: '已保存到 {symbol}', export_images: '导出 JSON 时包含截图', open_image: '打开图片', image_alt: '截图',
