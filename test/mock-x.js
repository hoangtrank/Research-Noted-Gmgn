...
function page() {
  const art = tw => `<article data-testid="tweet" role="article" tabindex="0">
    ...
    <div class="media" style="height:120px;background:linear-gradient(90deg,#1d4ed8,#7c3aed);border-radius:12px;margin:8px 0"></div>
  ...
  <body><nav><div>Home</div><div>Explore</div><div>Grok</div></nav><main>${TWEETS.map(art).join('')}</main><aside>Trends for you: Robinhood chain, AI agents, memecoins and other things people talk about today.</aside></body></html>`;
}
