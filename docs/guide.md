...
The default question is already written in your interface language and asks Grok to answer in this exact order: **DEV** first (confirmed dev/lead with X link, the token's account, GitHub/LinkedIn, anon or doxxed, past projects), then **KOL**, then **PROJECT** written in detail in 4 sections (the problem, the solution, how to use the platform in plain words for non-technical readers, the current state of the product), and finally **MEME** if it really is a meme. It also tells Grok to write "not found" instead of guessing, not to repeat the address/chain/chart, and to give X links in full form `https://x.com/...` so you can click them from inside the note.
